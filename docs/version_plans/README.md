...
当前下一步：6.0-A1 默认主交接入口仍为 [6.0_A1_HANDOFF_MASTER.md](./6.0_A1_HANDOFF_MASTER.md)。R1 readonly v2 runtime slice 已 stage accepted。Option B #54-#60 evidence chain 已完成：#54 deployed-code staleness corrected for R1 readonly v2 allowlist；#55 server-localhost `127.0.0.1:8088` three readonly endpoints passed；#56 public staging target unclear correctly blocked before request；#57 user-confirmed public IP `101.133.161.203` three readonly endpoints passed；#58 gate review concluded `ready_for_docs_sync_and_a1_closure_planning`；#59 synchronized current/status/index/handoff docs；#60 closure review decision 为 `accepted_with_observations`。A1 acceptance scope limited to R1 readonly v2 runtime slice + Option B deployed-code correction + server-localhost readonly verification + user-confirmed public IP readonly verification + docs sync posture。production verification 仍未执行，broad API smoke 仍未执行，Candidate C 尚未 implementation，R2 runtime completion and full v2 skeleton completion remain out of scope，A2 runtime has not started。下一步建议 `GPT-TASK: 6.0-A2 planning package / authorization prepackage gate`；该 gate 仅定义 A2 scope、evidence requirements 与 authorization boundaries，不启动 A2 runtime。production verification、broad API smoke、Candidate C、A2 runtime、Flutter / Android / release-chain work 仍是 separate future gates。后续 Codex 导出目录默认固定为 `C:\Users\zcxve\Downloads\`。

## 6.0 Alpha Version Split

- 6.0-A0：商用化底座与路线冻结版，planning-only，计划书已落地；仍需 Claude 轻量横向复评与 GPT 顾问最终验收
- 6.0-A1：后端 v2 与位置链路最小闭环；当前默认主交接入口为 [6.0_A1_HANDOFF_MASTER.md](./6.0_A1_HANDOFF_MASTER.md)，其他 A1 文件均为 supporting evidence / 补充附件，不创建多个 handoff 入口。R1 已新增极窄 readonly v2 runtime slice 并已 stage accepted。Option B #54-#60 evidence chain 已完成，#60 closure decision 为 `accepted_with_observations`，acceptance limited to current narrowed R1 readonly / Option B evidence chain。生产验证仍未执行，broad API smoke 仍未执行，Candidate C 尚未 implementation，R2 runtime completion and full v2 skeleton completion remain out of scope，A2 runtime has not started。当前仍禁止 Laravel upgrade、composer update、migration、production operation、API/write smoke、Flutter default base URL change、release chain 修改、production verification、broad API smoke、Candidate C implementation、未经授权扩展完整 v2 skeleton，以及 A2 runtime。

## 6.0-A1 Current Supporting Evidence

- [6.0_A1_POST_ACCEPTANCE_DOCS_SYNC_AND_A2_PLANNING_DECISION_REPORT.md](./6.0_A1_POST_ACCEPTANCE_DOCS_SYNC_AND_A2_PLANNING_DECISION_REPORT.md)：Issue #61 post-A1 current docs sync / A2 planning decision report；同步 #60 `accepted_with_observations` 到 current/status/index/handoff docs；下一步建议 `GPT-TASK: 6.0-A2 planning package / authorization prepackage gate`；不代表 A2 runtime start、production verification、broad API smoke、Candidate C implementation、full v2 skeleton completion 或 release-chain readiness。
- [6.0_A1_CLOSURE_REVIEW_ACCEPTANCE_REPORT.md](./6.0_A1_CLOSURE_REVIEW_ACCEPTANCE_REPORT.md)：Issue #60 closure review / acceptance report；decision `accepted_with_observations`；acceptance limited to current narrowed R1 readonly / Option B evidence chain；不代表 production readiness、production verification、broad API smoke、Candidate C implementation、full v2 skeleton completion 或 A2 start。
- [6.0_A1_DOCS_SYNC_AND_A1_CLOSURE_PLANNING_REPORT.md](./6.0_A1_DOCS_SYNC_AND_A1_CLOSURE_PLANNING_REPORT.md)：Issue #59 docs sync / A1 closure planning report；同步 current/status/index/handoff docs 到 #54-#58 evidence chain；下一步建议 `GPT-TASK: 6.0-A1 closure review / acceptance gate`；不代表 A1 final acceptance、production verification、broad API smoke、Candidate C implementation、A2 start 或 full v2 skeleton complete。
- [6.0_A1_GATE_REVIEW_AFTER_OPTION_B_PUBLIC_IP_PASS.md](./6.0_A1_GATE_REVIEW_AFTER_OPTION_B_PUBLIC_IP_PASS.md)：Issue #58 gate review after Option B public IP pass；结论 `ready_for_docs_sync_and_a1_closure_planning`。
- [6.0_A1_USER_CONFIRMED_PUBLIC_IP_READONLY_ENDPOINT_VERIFICATION_REPORT.md](./6.0_A1_USER_CONFIRMED_PUBLIC_IP_READONLY_ENDPOINT_VERIFICATION_REPORT.md)：Issue #57 user-confirmed public IP readonly endpoint verification；`http://101.133.161.203` 下三个 R1 readonly endpoints passed；不代表 production verification、broad API smoke 或 A1 final acceptance。
- [6.0_A1_PUBLIC_STAGING_READONLY_ENDPOINT_VERIFICATION_REPORT.md](./6.0_A1_PUBLIC_STAGING_READONLY_ENDPOINT_VERIFICATION_REPORT.md)：Issue #56 public staging readonly endpoint verification；staging target unclear，blocked before request；未执行 public staging endpoint request。
- [6.0_A1_OPTION_B_LOCALHOST_VERIFICATION_RERUN_REPORT.md](./6.0_A1_OPTION_B_LOCALHOST_VERIFICATION_RERUN_REPORT.md)：Issue #55 corrected Option B localhost verification rerun；`127.0.0.1:8088` 下三个 R1 readonly endpoints passed。
- [6.0_A1_OPTION_B_DEPLOYMENT_SYNC_EXECUTION_REPORT.md](./6.0_A1_OPTION_B_DEPLOYMENT_SYNC_EXECUTION_REPORT.md)：Issue #54 Option B deployment / sync execution report；deployed-code staleness corrected for seven allowlisted R1 readonly v2 files；route inventory showed three expected `api/v2` routes。
...
- [6.0_A1_IP_STAGING_AUTHORIZATION_CLAUDE_LIGHTWEIGHT_REVIEW.md](./6.0_A1_IP_STAGING_AUTHORIZATION_CLAUDE_LIGHTWEIGHT_REVIEW.md)：IP staging authorization Claude lightweight review；Claude verdict `pass with observations`，blockers none，recommended next step `proceed to user final parameter confirmation`；implementation 前需处理 `.env.staging` 创建方式、Nginx redacted content review、端口占用、安全组白名单、rollback health checks 等 observations；该 review 本身未 SSH、未修改服务器、未创建 IP staging、未执行 staging verification。
- [6.0_A1_IP_STAGING_OPTION_B_NARROW_SCAFFOLD_PREFLIGHT_REPORT.md](./6.0_A1_IP_STAGING_OPTION_B_NARROW_SCAFFOLD_PREFLIGHT_REPORT.md)：IP Staging Option B narrow scaffold / preflight report；已创建 `.env.staging` from `.env.example` 与 disabled localhost-only Nginx staging draft；`nginx -t` pass 但只验证 current active config；未创建 sites-enabled symlink，未 reload / restart，未执行 endpoint verification；staging 尚未启用、不可访问；recommended next step 为 `request sites-enabled symlink authorization`。
