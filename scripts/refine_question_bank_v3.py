...
SCENARIOS = {
    ...
    "loyalty": ["在“忠诚与自由”的平衡上，你更认同哪一边？", "关于异性社交边界，你更认同哪种做法？"],
    "integrity": ["当承诺与便利冲突时，你通常怎么做？", "在“说到做到”和“灵活调整”之间，你更看重哪一边？"],
    "goal_type": ["你开始一段关系时，通常抱着什么目标？", "如果描述你当前的恋爱方向，你会怎么选？"],
    "pace": ["从认识到确认关系，你更舒服的节奏是什么？", "双方都有好感时，你更偏好怎样推进？"],
    "exclusivity": ["进入稳定约会阶段后，你对“是否专一”的期待是什么？", "你更认同哪种建立排他关系的方式？"],
    "future_planning": ["关系前期，你会多早讨论未来规划（城市/婚育/职业）？", "面对“先聊未来还是先感受当下”，你更偏向哪一边？"],
...
INTRO_VARIANTS = ["", "回想最近一段真实经历，", "按你平时的做法，", "如果遇到这种情况，", "更贴近现实地说，"]
NEUTRAL_3 = ["会根据具体情况调整", "会结合关系阶段来决定", "会先观察再做决定", "会看双方状态灵活处理"]
...
def pick(items: list[str], key: str) -> str:
    return items[h(key) % len(items)]


def build_question_text(subtopic: str, qid: str) -> str:
    body = pick(SCENARIOS.get(subtopic, ["下面哪种状态更贴近你的真实情况？"]), qid + "_body")
    intro = pick(INTRO_VARIANTS, qid + "_intro")
...
def degree_templates(a: str, b: str, qid: str, seq: int) -> list[str]:
    strong_pool = [
        "我非常认同{X}",
        "我会坚定选择{X}",
        "遇到类似情况我通常会坚持{X}",
    ...
    si = (h(qid + "_s4_" + str(seq)) % len(strong_pool))
    mi = (h(qid + "_m4_" + str(seq)) % len(mild_pool))
    sj = (si + 3) % len(strong_pool)
    mj = (mi + 4) % len(mild_pool)
    return [
        strong_pool[si].format(X=a),
        mild_pool[mi].format(X=a),
        mild_pool[mj].format(X=b),
        strong_pool[sj].format(X=b),
    ]


def option_labels(a: str, b: str, qid: str, seq: int, n: int) -> list[str]:
    if n <= 1:
        return [a]
    if n == 2:
        return [a, b]
    if n == 3:
        neutral = NEUTRAL_3[(h(qid + "_n3") + seq) % len(NEUTRAL_3)]
        side_pool = [
            "{X}",
            "我会优先{X}",
            "遇到类似情况，我更可能{X}",
        ...
        a_tpl = side_pool[(h(qid + "_a3") + seq) % len(side_pool)].format(X=a)
        b_tpl = side_pool[(h(qid + "_b3") + seq + 3) % len(side_pool)].format(X=b)
        return [a_tpl, neutral, b_tpl]
    return degree_templates(a, b, qid, seq)


def process_file(path: Path) -> tuple[int, int]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    questions = payload.get("questions") or []
    seq_by_subtopic: dict[str, int] = defaultdict(int)
    changed = 0

    for q in questions:
        qid = str(q.get("question_id", ""))
        sub = str(q.get("subtopic", ""))
        seq = seq_by_subtopic[sub]
        seq_by_subtopic[sub] += 1
        ...
        q["question_text"] = {
            "zh": build_question_text(sub, qid + f"_{seq}"),
            "en": "Which option is closer to your real behavior?",
        }

        opts = q.get("options") or []
        labels = option_labels(a, b, qid, seq, len(opts))
        for i, opt in enumerate(opts):
            label = opt.get("label") or {}
            zh = labels[min(i, len(labels) - 1)]
            zh = zh.replace("愿意愿意", "愿意").replace("，，", "，").strip("，。 ")
            zh = zh.replace("更偏向", "偏向").replace("我在不少情况下会更偏向", "我在不少情况下会偏向")
            if zh.startswith("倾向") or zh.startswith("偏向"):
                zh = zh[2:] if zh.startswith("倾向") else zh[2:]
                zh = zh.strip("，。 ")
            label["zh"] = zh
            opt["label"] = label
        q["options"] = opts
