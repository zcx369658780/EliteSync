...
tmp_env="`$(mktemp)"
grep -vE '^(ANDROID_LATEST_VERSION_NAME|ANDROID_LATEST_VERSION_CODE|ANDROID_MIN_SUPPORTED_VERSION_NAME|ANDROID_DOWNLOAD_URL|ANDROID_CHANGELOG|ANDROID_APK_SHA256|ANDROID_FORCE_UPDATE|APP_URL)=' .env > "`$tmp_env" || true
cat >> "`$tmp_env" <<'EOF'
