...
- Current release: `0.04.04 / 40400`
- Current completed version: `4.5E`
- Current active planning target: `4.6 RTC / 通话基础设施版（语音优先）` is in `4.6A` boundary-freeze and dependency-mapping stage.
...
- Remote SSH execution rule: always call explicit `C:\WINDOWS\System32\OpenSSH\ssh.exe` with an argument array (or raw `--%` passthrough when needed); do not rely on PowerShell aliases or unescaped `ssh -o ...` forms, because PowerShell can misparse `-o` / `-i` and break server-side checks.
- `4.6P` has reached live voice success: phone speech is audible on the emulator; the plan should now be treated as `pass with observations` with only a final reverse-direction mic retest left as an observation item, not a blocker.
...
- `4.5A` is the notification-and-social-conversion boundary freeze: the notification center, unread count, read/all-read actions, and lightweight return paths into chat/status/match/questionnaire history/settings are in place, but do not expand it into a push platform, RTC, online status system, or message/status/match main-chain rewrite. `4.5E` has now completed the stable live notification-center page screenshot and is formally收口.
- `4.6` is the RTC / call infrastructure release (voice-first). It is currently in `4.6A` boundary freeze and dependency mapping. The scope is only a 1v1 minimal real-time call loop; do not expand it into multi-party calls, live rooms, online presence, or push-platform work. Keep local work frontend/UI/doc-only; keep backend writes, migrations, backups, restores, and production DB recovery on Aliyun only.
- `4.6F` is the LiveKit voice-media subtask for 4.6: preserve the existing RTC state machine and only add the voice media layer, join-info endpoint, and Flutter connection shell. Do not expand it into multi-party / live / presence platforms. Current reality: calls can connect and end, but real audio is still not closed; the waveform diagnostic stays on waiting-remote-track, so the material should be handed to GPT advisor for ruling instead of blindly extending the chain.
- Aliyun has already brought up a self-hosted LiveKit container and written the `LIVEKIT_*` env vars into backend `.env`; `/api/v1/rtc/calls/{callId}/livekit` returns usable join-info. If work continues, it should only be after advisor ruling and only as a minimal closure pass, not a RTC state-machine rewrite.
- `4.6B` has now landed the RTC minimal loop: backend RTC sessions / events / state machine are implemented, Flutter call entry and call page skeletons are implemented, and notification-center call routing is available. The current recommended acceptance wording is `pass with observations`.
- `4.6C` has now added the incoming-call page and call-result page skeletons, and notification-center routing now splits rtc_call invite/terminal flows into the appropriate pages while still keeping the 1v1 voice-first boundary.
- `4.6D` has now added the microphone permission gate and recovery shell: chat-room call entry checks permission first, and if denied it routes to a permission page with request / system-settings recovery actions. The scope is still 1v1 voice-first and must not expand into multi-party / live / online-presence work.
...
- On 2026-04-21 the user re-confirmed the smoke account passwords: `17094346566` / `SmokeUser` uses `1234567aa`, and `13772423130` / `test1` uses `zcx658023`. Use these as the default smoke/regression credentials until explicitly changed again.
...
- 2026-04-21 snapshot: release baseline is `0.04.04 / 40400`; `4.4S` media-chain stability repair is formally archived and `4.5E` notification-center live evidence is complete. The last CI regression on `AppVersionApiTest` was caused by `.env.example` still carrying the old Android version defaults; those defaults have been corrected back to `0.04.04 / 40400` and the test now passes. Root temp screenshots / sample video should be treated as disposable artifacts, not as long-term memory content.
- RTC disconnect handling now uses a backend heartbeat contract instead of relying only on explicit hangup: `POST /api/v1/rtc/calls/{callId}/heartbeat` refreshes `initiator_last_seen_at` / `peer_last_seen_at`, and the backend auto-ends `connecting` / `in_call` sessions after 10 seconds of participant inactivity to avoid stale busy states.
